...
router.get("/calculate", async (req, res) => {
  try {
    ...
    // Fetch daily logs with photo analysis
    ...
    // Get photo metrics trend
    const photoMetricsTrend = getPhotoMetricsTrend(dailyLogs, 30);

    // Get most recent photo analysis
    const recentPhotoLog = dailyLogs.find(
      (log) => log.photoUrl && log.acneSeverity !== null
    );
    ...
    res.json({
      ...
      insights: {
        ...
        photoMetrics: photoMetricsTrend,
...
router.get("/latest", async (req, res) => {
  try {
    ...
    if (!latestRisk) {
      return res
        .status(404)
        .json({ error: "No risk scores found. Please log cycle data first." });
    }
